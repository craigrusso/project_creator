...
root = Tk()
root.title("CR2 Creative // Project creator v.14")
...
#define filenames to store data for later
pathFilename = 'path'
templatePathFilename = 'template'

#see if the path file exsists if not create it
try:
	rootPath = pickle.load(open(pathFilename, "rb"))	
except (OSError, IOError) as e:
	rootPath = ""
	pickle.dump(rootPath, open(pathFilename, "wb"))

#see if the template file exists if not create it
try:
	tempFileFullPath = pickle.load(open(templatePathFilename, "rb"))	
except (OSError, IOError) as e:
	tempFileFullPath = ""
	pickle.dump(tempFileFullPath, open(templatePathFilename, "wb"))


#Write Globals to pickle
